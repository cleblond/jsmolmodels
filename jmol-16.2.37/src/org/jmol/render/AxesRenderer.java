...
public class AxesRenderer extends CageRenderer {
  ...
  private void render1(Axes axes,int mad10, boolean isXY,
    ...
    boolean checkAxisType = (labels2 == null && axisType != null && (isXY 
    		|| vwr.getFloat(T.axesoffset) != 0 || axes.fixedOrigin != null));
    if (isXY) {
      if (mad10 >= 20) {
      ...
      for (int i = nPoints; --i >= 0;) {
        P3 p = axes.getAxisPoint(i, !isDataFrame, pointT);
        tm.transformPtNoClip(p, p3Screens[i]);
      }
      if (shifting) {
        if (vvert == null) {
          vvert = new P3[] { new P3(), new P3(), new P3(), new P3(),
              new P3(), new P3() };
        }
        if (shiftA) {
          setVVert(axes, 0);
    ...
    for (int i = nPoints; --i >= 0;) {
      ...
      if (label != null && label.length() > 0) {
        P3 p = (i == 0 && shiftA ? vvert[1] : i == 1 && shiftB ? vvert[3]
