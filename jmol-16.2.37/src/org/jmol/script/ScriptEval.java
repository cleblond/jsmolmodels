...
//
  ...
  public void resumeEval(Object sco) {
    ...
    switch (executeCommands(null,
        thisContext != null && thisContext.isTryCatch, scriptLevel <= 0)) {
    case EXEC_ASYNC:
      break;
    case EXEC_ERR:
    case EXEC_OK:
  ...
  private Object evaluate(Object expr, boolean asVariable, boolean compileOnly) {
    try {
      if (expr instanceof String) {
        ...
        // in JavaScript legacy we are just checking for [][] 
...
//            + " -- STATEMENT CONTAINING @{} SKIPPED");
  ...
  private void cmdMoveto() throws ScriptException {
    ...
    case T.plane:
      ...
      v.cross(x, v);
      q =  Quat.getQuaternionFrame(null, v, x).inv();
      i = iToken + 1;
      break;
    case T.axis:
      ...
      default:
        ...
        if (uc == null) {
          uc = vwr.getSymTemp().setUnitCellFromParams(null, false, Float.NaN);
        }
  ...
  public void cmdRotate(boolean isSpin, boolean isSelected)
    ...
    float degreesPerSecond = JC.FLOAT_MIN_SAFE;
    int nPoints = 0;
    ...
    float rate = (degreesPerSecond == JC.FLOAT_MIN_SAFE ? 10
    ...
    if (isSpin && translation != null
      ...
      endDegrees = 0.01f;
      rate = (degreesPerSecond == JC.FLOAT_MIN_SAFE ? 0.01f
          : degreesPerSecond < 0 ?
          // -n means number of seconds, not degreesPerSecond
          -endDegrees / degreesPerSecond
    ...
    if (zoom == 0 || currentZoom == 0) {
      ...
      if (bs == null) {
        ...
        case T.unitcell:
          ...
          r = 0;
          for (int j = 0; j < 8; j++) {
            float d = pts[j].distanceSquared(center);
            if (d > r)
              r = d;
          }
          if (r == 0)
