...
public class SV extends T implements JSONEncodable {
  ...
  public final static SV vNaN = newSV(decimal, Integer.MAX_VALUE, Float.valueOf(Float.NaN));
  ...
  // this method is legacy transpiler-specific, not in SwingJS
    ...
    case matrix4f:
      ...
      if (pt1 != 0 && Math.abs(pt1) <= len
        ...
        Lst<SV> sv = var.getList();
          ...
          case point3f:
            P3 p = (P3) var.value;
            data = new float[] { p.x, p.y, p.z };
            break;
          case point4f:
            P4 p4 = (P4) var.value;
            data = new float[] { p4.x, p4.y, p4.z, p4.w };
            break;
          }
          if (pt1 > 0) {
            if (tok == matrix3f)
  ...
  public static boolean areEqual(SV x1, SV x2) {
    ...
    return (x1.isNaN() ? x2.isNaN() : Math.abs(fValue(x1) - fValue(x2)) < 0.000001);
  }
