...
public class MathExt {
    ...
    case 1:
      ...
      case T.string:
        ...
        if (!s.equals("h") && !s.equals("r") && (s.indexOf(">") >= 0 || s.indexOf(",") < 0 || s.indexOf(":") > 0 )) {
          ...
          m4 = (M4) vwr.getSymTemp().convertTransform(s, null);
        }
        break;
      case T.varray:
  ...
  private String matToString(M4 m4, boolean asABC) {
    return (asABC ?
  ...
  private boolean evaluateSpacegroup(ScriptMathProcessor mp, SV[] args) {
    ...
    case 2:
      ...
      if (args[1].tok != T.string) {
        ...
        params = ucParams = SimpleUnitCell.newParams(ucParams, Float.NaN);
      }
      //$FALL-THROUGH$
    case 1:
      ...
      if (itaNo > 0 || args[0].tok == T.decimal || args[0].tok == T.string) {
        ...
        if (itaNo > 0 || xyzList.length() > 1 && xyzList.charAt(1) == '/'
            || !xyzList.endsWith(":") && !Double.isNaN(PT.parseFloat(xyzList)))
          xyzList = "ITA/" + xyzList;
  ...
  private boolean evaluatePointGroup(ScriptMathProcessor mp, SV[] args,
    ...
    SymmetryInterface pointGroup = vwr.getSymTemp().setPointGroup(vwr, null,
        ...
        distanceTolerance < 0
            ...
            : distanceTolerance,
        linearTolerance < 0 ? vwr.getFloat(T.pointgrouplineartolerance)
            : linearTolerance, 
    ...
    if (ucnew == null) {
      ...
      if (haveUC) {
        ...
        case 1:
          ...
          if (l != null && l.size() == 3) {
            for (int i = 0; i < 3; i++)
              ucnew[i + 1] = P3.newP(SV.ptValue(l.get(i)));
            break;
          }
          //$FALL-THROUGH$
        default:
  ...
  private boolean evaluateFind(ScriptMathProcessor mp, SV[] args)
    ...
    if (isList || isPattern) {
      PatternMatcher pm = (isPattern ? getPatternMatcher() : null);
  ...
  private boolean evaluateFormat(ScriptMathProcessor mp, int intValue,
    ...
    case SV.FORMAT_XYZ:
    case SV.FORMAT_ABC:
      return (x.tok == T.matrix4f && mp.addXStr(matToString((M4) x.value, pt == SV.FORMAT_ABC)));
    default:
//    case SV.FORMAT_JSON:
    ...
    case T.measure:
      ...
      Object obj = (vwr.newMeasurementData(null, points))
          .set(0, null, rd, property, strFormat, units, null, isAllConnected,
              isNotConnected, null, true, 0, (short) 0, null, Float.NaN, null)
  ...
  private boolean evaluatePlane(ScriptMathProcessor mp, SV[] args, int tok, boolean isSelector)
    ...
    case 1:
      if (tok == T.plane) {
        ...
        plane = P4.new4(ptuvw.x, ptuvw.y, ptuvw.z, 0);
        if (isSelector) {
          break;
        }
      }
    ...
    case 4:
      ...
      case T.hkl:
        ...
        plane = e.getHklPlane(P3.new3(SV.fValue(args[0]),
    ...
    if (isSelector) {
        ...
        case T.bitset:
          ...
          for (int i = bsAtoms.nextSetBit(0); i >= 0; i = bsAtoms.nextSetBit(i + 1)) {
            P3 p = new P3(); // will be filled
            Measure.getPlaneProjection(vwr.ms.at[i], plane, p, new V3());
...
//  "xyzOriginal", 
  ...
  private Object getSymopInfo(ScriptMathProcessor mp, SV x1, SV[] args, int index, boolean isProperty) throws ScriptException {
    ...
    if (narg == 2 && args[0].tok == T.matrix4f && args[1].tok == T.string
      ...
      return vwr.getSymStatic().staticConvertOperation("", (M4) args[0].value);
    }
    ...
    if (pt1 != null && pt2 == null && bs1 != null && !bs1.isEmpty()) {
      pt2 = pt1;
      pt1 = P3.newP(vwr.ms.at[bs1.nextSetBit(0)]);
    }
    ...
    if (isWyckoff) {
      P3 pt = (haveAtom ? vwr.ms.getAtom(iatom) : pt1);
  ...
  private PatternMatcher getPatternMatcher() {
    return (pm == null
        ? pm = (PatternMatcher) Interface.getUtil("PatternMatcher", e.vwr, "script")
        : pm);
  }
