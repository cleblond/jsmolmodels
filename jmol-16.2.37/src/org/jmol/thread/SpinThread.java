...
public class SpinThread extends JmolThread {
  ...
  public int setManager(Object manager, Viewer vwr, Object params) {
    ...
    if (options == null) {
      ...
      endDegrees = ((Number) options[0]).floatValue();
    ...
    while (true)
      ...
      case CHECK1: // cycling
        ...
        if (nFrames >= 0 && index >= nFrames 
            || (endDegrees >= 1e10f ? nDegrees/endDegrees > 0.99f 
            : !isNav && endDegrees >= 0 ? nDegrees >= endDegrees - 0.001
            : -nDegrees <= endDegrees + 0.001f)) {
          isDone = true;
  ...
  private void doTransform() {
    if (centerAndPoints != null) {
      ...
      if (transformManager.spinX != 0) {
        transformManager.rotateXRadians((float) (transformManager.spinX
            * JC.radiansPerDegree / myFps), null);
      }
      if (transformManager.spinY != 0) {
        transformManager.rotateYRadians((float) (transformManager.spinY
            * JC.radiansPerDegree / myFps), null);
      }
      if (transformManager.spinZ != 0) {
        transformManager.rotateZRadians((float) (transformManager.spinZ
