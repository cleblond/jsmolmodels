/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2024-10-08 18:58:53 -0500 (Tue, 08 Oct 2024) $
 * $Revision: 22635 $
...
public class ModelSet extends BondCollection {
  ...
  private Object calculatePointGroupForFirstModel(BS bsAtoms, boolean doAll,
    ...
    pointGroup = symmetry.setPointGroup(vwr, pointGroup, center, pts,
        bs, haveVibration,
        (isPoints ? 0 : vwr.getFloat(T.pointgroupdistancetolerance)), vwr.getFloat(T.pointgrouplineartolerance), (bs == null ? pts.length : bs.cardinality()), localEnvOnly);
  ...
  public BS addHydrogens(Lst<Atom> vConnections, P3[] pts) {
    ...
    for (int i = 0, n = am[modelIndex].act + 1; i < vConnections.size(); i++, n++) {
      ...
      Atom atom2 = addAtom(modelIndex, atom1.group, 1, "H" + n, null, n,
          ...
          null, false, false, (byte) 0, null, Float.NaN);
...
//  }
  ...
  public float calcRotationRadius(int modelIndex, P3 center, boolean useBoundBox) {
    ...
    for (int i = ac; --i >= 0;) {
      ...
      float d = center.distance(atom) + getRadiusVdwJmol(atom);
      if (d > maxRadius)
        maxRadius = d;
    }
  ...
  private void calcUnitCellMinMax() {
    ...
    for (int i = 0; i < mc; i++) {
      ...
      if (uc == null || !uc.getCoordinatesAreFractional())
        continue;
      P3[] vertices = uc.getUnitCellVerticesNoOffset();
      P3 offset = uc.getCartesianOffset();
      for (int j = 0; j < 8; j++) {
...
//      averageAtomPoint = getAtomSetCenter(vwr.getAllAtoms());
    ...
    for (int i = bsA.nextSetBit(0); i >= 0; i = bsA.nextSetBit(i + 1)) {
      ...
      while (iter.hasNext()) {
        ...
        if (isH && !Float.isNaN(C.x) && !Float.isNaN(D.x)) {
          ...
          //
          energy = HBond.calcEnergy((float) Math.sqrt(d2), C.distance(atom),
  ...
  public Atom addAtom(int modelIndex, Group group, int atomicAndIsotopeNumber,
                      ...
                      boolean isHetero, boolean isNegDisorder, byte specialAtomID, BS atomSymmetry, float bondRadius) {
    Atom atom = new Atom().setAtom(modelIndex, ac, xyz, radius, atomSymmetry,
        atomSite, (short) atomicAndIsotopeNumber, formalCharge, isHetero, false);
    am[modelIndex].act++;
...
//        }
    ...
    return (type == T.volume ? vMin + "\t{" + dx + " " + dy + " " + dz + "}\t" + bsAtoms
        : type == T.unitcell ? null 
        		: q == null || q.getTheta() == 0 ? new Quat() : q);
  }
    ...
    for (int i = mc; --i >= 0;) {
      ...
      if (am[i].isBioModel) {
        ...
        if (conformationIndex < -1000) {
          ...
          c0 = -conformationIndex;
          float[] parts = (float[]) vwr.getDataObj("property_part", bsAtoms, JmolDataManager.DATA_TYPE_AF);
           if (parts == null) {
             c0 -= 1;
           } else {
  ...
  private void fixP1AtomSites(SymmetryInterface sym, BS bsAtoms) {
    ...
    A4[] baseAtoms = new A4[n];
    int nbase = 0;
    double slop2 = sym.getPrecision();
    slop2 *= slop2;
    for (int i = bsAtoms.nextSetBit(0); i >= 0; i = bsAtoms.nextSetBit(i + 1)) {
      Atom a = at[i];
      // just using A4 here as a convenience
      A4 p = new A4();
      p.setT(a);
