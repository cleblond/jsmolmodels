/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2024-04-10 17:30:25 -0500 (Wed, 10 Apr 2024) $
 * $Revision: 22611 $
 *
...
public class Measures extends AtomShape implements JmolMeasurementClient {
  ...
  private void defineAll(int iPt, Measurement m, boolean isDelete,
    ...
    define((new MeasurementData().init(null, vwr, points)).set(tokAction, htMin, radiusData, m.property, strFormat, null, tickInfo,
        mustBeConnected, mustNotBeConnected, intramolecular, true, 0, (short) 0, null, Float.NaN, null),
  ...
  public void processNextMeasure(MeasurementData md, Measurement m) {
    ...
    if (md.tokAction == T.select) {
      if (iThis < 0 || !isInRange(Float.NaN, m))
    	  return;
       bsSelected.set(iThis);
    }
    if (iThis >= 0) {
      if (tokAction == T.delete) {
        if (isInRange(Float.NaN, m))
        deleteI(iThis);
  ...
  private boolean isInRange(float val, Measurement m) {
